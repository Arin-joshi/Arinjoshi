...
const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  const glowDarkRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    // Lerp tracking for smooth background glow
    let targetX = 0;
    let targetY = 0;
    let currentX = 0;
    let currentY = 0;
    ...
    const getWebGLContext = (canvasElement: HTMLCanvasElement) => {
      ...
      if (!isWebGL2) {
        gl = (canvasElement.getContext('webgl', params) ||
          canvasElement.getContext('experimental-webgl', params)) as WebGLRenderingContext | null;
      }
      if (!gl) {
    ...
    try {
      ...
      console.warn('WebGL fluid simulation cursor skipped:', e);
      return;
    }
    ...
    function createFBO(w: number, h: number, internalFormat: number, format: number, type: number, param: number): FBO {
      ...
      const fbo = gl.createFramebuffer();
      if (!fbo) throw new Error('Failed to create FBO framebuffer');
      gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texture, 0);
      gl.viewport(0, 0, w, h);
      gl.clear(gl.COLOR_BUFFER_BIT);
    ...
    function updateFrame() {
      ...
      // Update background glow coordinates with high-fidelity inertia (lerping)
      ...
      if (glowLightRef.current) {
        glowLightRef.current.style.transform = transformStr;
        glowLightRef.current.style.opacity = currentOpacity.toFixed(4);
      }
      if (glowDarkRef.current) {
        glowDarkRef.current.style.transform = transformStr;
        glowDarkRef.current.style.opacity = currentOpacity.toFixed(4);
      }

      // suspension logic for CPU/GPU efficiency when idle
      const timeSinceLastInput = Date.now() - lastInputTime;
      // keep loop running slightly longer to let the background orb settle
      if (timeSinceLastInput > 4000 && Math.abs(currentX - targetX) < 1 && Math.abs(currentY - targetY) < 1) {
        isLoopRunning = false;
    ...
    function resizeCanvas() {
      ...
      const width = scaleByPixelRatio(rect.width || window.innerWidth);
      const height = scaleByPixelRatio(rect.height || window.innerHeight);
    ...
    // Coordinates check relative to canvas bounds
    function getPointerPosition(clientX: number, clientY: number) {
      const rect = canvas.getBoundingClientRect();
      const mouseX = clientX - rect.left;
      const mouseY = clientY - rect.top;

      if (mouseY >= 0 && mouseY <= rect.height && mouseX >= 0 && mouseX <= rect.width) {
        return {
          x: mouseX,
          y: mouseY,
          valid: true
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      if (pos.valid) {
        ...
        if (!firstMouseMoveHandled) {
          const color = generateColor();
          updatePointerMoveData(pointer, scaleByPixelRatio(pos.x), scaleByPixelRatio(pos.y), color);
    ...
    // Initial wake up to render initial canvas frame and bind sizes
    wakeUpLoop();
  ...
  return (
    <>
      ...
      <div
        ref={glowLightRef}
        className="absolute pointer-events-none z-0 dark:hidden"
        style={{
          width: '180px',
          height: '180px',
          borderRadius: '50%',
          ...
          background: 'radial-gradient(circle, rgba(139, 92, 246, 0.22) 0%, rgba(239, 68, 68, 0.1) 40%, transparent 70%)',
          filter: 'blur(40px)',
      ...
      <div
        ref={glowDarkRef}
        className="absolute pointer-events-none z-0 hidden dark:block"
        style={{
          width: '200px',
          height: '200px',
          borderRadius: '50%',
          ...
          left: 0,
          transform: 'translate3d(0px, 0px, 0) translate(-50%, -50%)',
          background: 'radial-gradient(circle, rgba(139, 92, 246, 0.16) 0%, rgba(239, 68, 68, 0.08) 35%, rgba(16, 185, 129, 0.03) 60%, transparent 70%)',
          filter: 'blur(35px)',
          willChange: 'transform',
          opacity: 0
        }}
      ...
      <canvas
        ref={canvasRef}
        className="absolute inset-0 w-full h-full pointer-events-none z-10 mix-blend-normal dark:mix-blend-screen opacity-80 dark:opacity-65"
      />
    </>
  );
};
